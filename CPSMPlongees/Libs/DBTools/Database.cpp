...
int countField(QSqlDatabase& db,const QString& table,const QString& field,QVariant value)
{
    ...
    if(!query.next())//if nothing was found
    {
        ...
        {
            qDebug() << __PRETTY_FUNCTION__ << " : Cannot satisfy request : <" << query.last()<<">";
        }
    }
    ...
    {
        QString errStr{QString{"%0 : SQL error : %1"}.arg(__PRETTY_FUNCTION__,err.text())};
        qCritical() << errStr;
        return -1;
    }

    if constexpr(kDebugQueries)
        qDebug() << __PRETTY_FUNCTION__ << query.value(0);
    return 0;
}
