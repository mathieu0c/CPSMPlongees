...
DiveEdit::DiveEdit(QWidget *parent) : QWidget(parent), ui(new Ui::DiveEdit) {
  ...
  connect(ui->pb_time_preset_morning, &QPushButton::clicked, this,
          [this]() { ui->te_diveTime->setTime(QTime(9, 0)); });
  connect(ui->pb_time_preset_afternoon, &QPushButton::clicked, this,
          [this]() { ui->te_diveTime->setTime(QTime(15, 0)); });
  connect(ui->pb_time_preset_sunset, &QPushButton::clicked, this,
          [this]() { ui->te_diveTime->setTime(QTime(20, 0)); });
  connect(ui->pb_time_preset_night, &QPushButton::clicked, this,
          [this]() { ui->te_diveTime->setTime(QTime(23, 0)); });

  connect(ui->te_diveTime, &QTimeEdit::timeChanged, this,
          [this](const QTime &time) {
            ui->te_diveTime->setStyleSheet(QString{"background-color: %0"}.arg(
                cpsm::BackgroundColorFromTime(time).name(QColor::HexRgb)));
          });

  connect(ui->buttonBox, &QDialogButtonBox::accepted, this, &DiveEdit::OnOk);
  connect(ui->buttonBox, &QDialogButtonBox::rejected, this,
          &DiveEdit::OnCancelled);

  ui->diverSearch_global->SetModel(&m_non_members_model);
  ui->diverSearch_dive->SetModel(&m_members_model);

  connect(&m_members_model, &cpsm::DiveEditMembers::DiveMembersChanged, this,
          &DiveEdit::OnDiveMembersChanged);
  connect(&m_members_model, &cpsm::DiveEditMembers::SetSelectedRows,
          ui->diverSearch_dive, &DiverSearch::SetSelectedRows);

  connect(ui->pb_non_members_to_members, &QPushButton::clicked, this,
          &DiveEdit::MoveSelectedDiversFromNonMembersToMembers);
  connect(ui->diverSearch_global, &DiverSearch::DoubleClickOnDiver, this,
          &DiveEdit::MoveSelectedDiversFromNonMembersToMembers);
  connect(ui->pb_members_to_non_members, &QPushButton::clicked, this,
          &DiveEdit::MoveSelectedDiversFromMembersToNonMembers);
  connect(ui->diverSearch_dive, &DiverSearch::DoubleClickOnDiver, this,
          &DiveEdit::MoveSelectedDiversFromMembersToNonMembers);

  /* Dive edit connections */
  connect(ui->de_diveDate, &QDateEdit::dateChanged, this,
          [this](const QDate &date) { m_dive.dive.datetime.setDate(date); });
  connect(ui->te_diveTime, &QTimeEdit::timeChanged, this,
          [this](const QTime &time) { m_dive.dive.datetime.setTime(time); });
  connect(ui->cb_diveSite, &QComboBox::currentIndexChanged, this,
          [this](int index) {
            m_dive.dive.diving_site_id =
                ui->cb_diveSite->itemData(index).toInt();
          });
}

DiveEdit::~DiveEdit() { delete ui; }
...
void DiveEdit::RefreshNonMembersFromDB() {
  /* Reload divers from DB while keeping existing filters (e.g. hide-members
   * filter) intact */
  m_non_members_model.LoadFromDB();
}
...
void DiveEdit::RefreshFromDB() {
  auto level_list{db::readLFromDB<cpsm::db::DivingSite>(
      db::Def(), cpsm::db::ExtractDivingSite, "SELECT * FROM %0",
...
bool DiveEdit::WasEdited() const { return m_dive != m_original_dive; }
...
void DiveEdit::OnDiveMembersChanged() {
  ...
  if (m_previous_dive_members !=
      kMembersDiversIds) { /* Only change (reapply) the filter if the divers ids
                              were changed */
...
void DiveEdit::RefreshDiveMembers() {
  ...
  for (const auto &[diver_id, dive_member] :
       m_members_model.GetDivingMembers()) {
    std::ignore = diver_id;
