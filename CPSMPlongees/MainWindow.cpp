...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->tabw_main, &QTabWidget::currentChanged, this, [this](int index) {
    if (index == MainTabs::kDives && ui->tab_dives->currentIndex() == DiveTabPages::kEditDive) {
      ui->pg_editDive->RefreshNonMembersFromDB();
    }
    if (index == MainTabs::kDivers && ui->tab_divers->currentIndex() == DiverTabPages::kEditDiver) {
...
void MainWindow::OnDiverEdited(std::optional<std::tuple<cpsm::db::Diver, cpsm::db::DiverAddress>> edit_opt) {
  ...
  SPDLOG_ERROR(
      "Failed to save driver: <{}> and address <{}> to "
      "database.\nErrCode=<{}>\nError: <{}>",
      diver,
      address,
...
void MainWindow::OnDiveEdited(std::optional<cpsm::db::DiveAndDivers> edit_opt) {
  ...
  SPDLOG_ERROR(
      "Failed to save dive and its divers: <{}> to "
...
void MainWindow::on_pb_deleteDiver_clicked() {
  ...
  if (!database.transaction()) {
    success = false;
    SPDLOG_ERROR(
        "Failed to remove diver from database (Failed to start "
        "transaction): {}",
        database.lastError());
...
void MainWindow::on_pb_newDiver_clicked() {
  ...
  default_diver.diver.nitrox_diver_level_id = 1;
  default_diver.diver.birth_date = QDate::currentDate().addYears(-30);
  default_diver.diver.certif_date = QDate::fromJulianDay(0);
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  const auto kAnswer{QMessageBox::warning(this,
                                          tr("Modifications non sauvegardées"),
